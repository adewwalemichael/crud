...
#
# # create label entry widgets, and buttons for adding a user
...
#     def add_user():
#      try:
#         conn = mysql.connector.connect(host="Localhost", user="root", password="", database="crud", cursor= conn.cursor())
#
#         # CREATE   TABLE
#         cursor.execute(
#             "CREATE TABLE IF NOT EXIST users"
#             " (id int auto increment primary key,firstName TEXT,lastName TEXT, email TEXT)")
#
#         # insert users data into the database
#
#         insert_query = "INSERT INTO users(fistName, lastName,email) VALUE ( %s, %s %s)"
...
#         messagebox.showerror("Error", f"Fail to add user")
#
#      finally:
...
#
#
# # create a button to add the users
#     add_button = Button(add_user_win, text="Add User", command= add_user)
#     add_button.pack()
#
# def delete_user_widow():
#     delete_user_win = Toplevel()
#     delete_user_win.title("Delete Uesr")
#
...
#  # def delete_user():
#  #         try:
#  #             conn= mysql.connector.connet(host="localhost",user="root",password="", database="crud",cursor= conn.cursor)
#
#
#
#
...
#     background_image = Image.open("backgroung.jpeg")
...
def add_user_window():
    ...
    def add_user():
        try:
            conn = mysql.connector.connect(host="localhost", user="root", password="", database="crud") #record
            cursor = conn.cursor()
